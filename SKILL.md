---
name: email_sender
description: Send email through SMTP providers such as 126, QQ, Sina, or Aliyun Mail using an SMTP authorization code or mailbox password. Use when the user wants Codex to scaffold SMTP mail sending, configure SMTP credentials, or send a simple test email.
---
...
Put the SMTP credentials in the following embedded configuration block.
The sending script reads this block directly from `SKILL.md`.
...
- configure SMTP credentials for a mailbox account such as `126`, `qq`, `sina`, or `aliyun`
...
1. Read the embedded config block in this file. By default, the script resolves `SKILL.md` relative to the script's own skill directory, not the current working directory.
...
- Before a real send, make sure `FROM_EMAIL` and the runtime message inputs are not placeholder values unless the user explicitly wants a dry run.
...
For preset providers, the minimum practical configuration inside `SKILL.md` is:
...
- Custom provider: set `SMTP_HOST`, `SMTP_PORT`, and `SMTP_USE_SSL` explicitly in the embedded config block.

## Example Requests

- "Send a test email to `someone@example.com` through QQ SMTP."
- "给 `xxx@xxx.com` 发一个主题为 `xxx` 内容为 `xxx` 的邮件。"
- "Set up SMTP credentials for a 126 mailbox."
